...
var (
	// errInvalidCredentials is an error that happens when user's password is invalid
	errInvalidCredentials = fmt.Errorf("invalid credentials")
	// errConnectionTimeout is an error that happens when on one LDPA endpoint responds .
	errConnectionTimeout = fmt.Errorf("connection timeout")
	// errMissedUsername is na error that happens
...
func (cli *Client) FindOIDCClaims(ctx context.Context, username string) (map[string]interface{}, error) {
	...
	// 	return nil, err
	// }
	// fmt.Println("Oidc entries ", entries)
...
func (cli *Client) findBasicUserDetails(cn conn, username string, attrs []string) (map[string]interface{}, error) {
	if cli.BindDN != "" {
		...
		if err := cn.Bind(cli.BindDN, cli.BindPass); err != nil {
			return nil, errors.Wrap(err, "Failed to login to a LDAP with a service account")
		}
	}
...
func (c *ldapConnector) Connect(ctx context.Context, addr string) (conn, error) {
	...
	if c.IsTLS {
		tcpcn, err := ldap.DialTLS("tcp", addr, &tls.Config{InsecureSkipVerify: true})
		if err != nil {
			return nil, err
		}
		fmt.Println("Connection on ldaps")
		return &ldapConn{Conn: tcpcn, BaseDN: c.BaseDN, RoleBaseDN: c.RoleBaseDN}, nil
	} else {
		tcpcn, err := ldap.Dial("tcp", addr)
		if err != nil {
			return nil, err
		}
		fmt.Println("Connection on ldap")
		return &ldapConn{Conn: tcpcn, BaseDN: c.BaseDN, RoleBaseDN: c.RoleBaseDN}, nil
	}
}
...
func (c *ldapConn) SearchUserRoles(user string, attrs ...string) ([]map[string]interface{}, error) {
	query := fmt.Sprintf("(|"+"(&(|(objectClass=group)(objectClass=groupOfNames))(member=%[1]s))"+"(&(objectClass=groupOfUniqueNames)(uniqueMember=%[1]s))"+")", user)
...
//APP_Endpoints=172.16.21.151:636 APP_BaseDN="dc=meet,dc=jitsi" APP_Role_BaseDN="ou=Users,dc=meet,dc=jitsi" APP_BindDN="cn=Tushar Sonawane,cn=Users,dc=meet,dc=jitsi" APP_BindPW="Pooja@123" go run main.go
